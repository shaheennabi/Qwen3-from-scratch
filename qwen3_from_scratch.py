"""Lightweight Qwen 3 model implementation with detailed tensor shape documentation.

This module implements rotary position embedding (RoPE), RMS normalization, group-query attention,
feed-forward networks, transformer blocks, tokenizer wrappers, and HF weight loading. Every
major operation is explained with explicit tensor shape comments to aid debugging and learning.
"""
...
def compute_rope_angles(head_dim, theta_base=10000, context_length=2048, dtype=torch.float32):
    """Compute RoPE sin/cos tables for rotary position embeddings.

    Args:
        head_dim: Attention head dimension, must be even.
        theta_base: Base frequency constant for RoPE.
        context_length: Maximum sequence length.
        dtype: Tensor dtype for outputs.

    Returns:
        cos: Tensor of shape (context_length, head_dim // 2).
        sin: Tensor of shape (context_length, head_dim // 2).
    """

    assert head_dim % 2 == 0, "head_dim must be even"

    index = torch.arange(0, head_dim, 2, dtype=dtype)  # (head_dim/2,)
    inv_freq = 1.0 / (theta_base ** (2 * index / head_dim))  # (head_dim/2,)
    ...
    cos = torch.cos(angles)  # (context_length, head_dim/2)
    sin = torch.sin(angles)  # (context_length, head_dim/2)
...
def apply_rope(x, cos, sin):
    ...
    Args:
        x: Tensor with shape (B, T, num_heads, head_dim).
        cos: Tensor with shape (T, head_dim/2) in training or (head_dim/2,) in inference.
        sin: Tensor with shape (T, head_dim/2) in training or (head_dim/2,) in inference.
    ...
    if cos.dim() == 2:  # training mode: cos shape = (T, D/2)
        cos = cos.unsqueeze(0).unsqueeze(2)  # (1, T, 1, D/2)
        sin = sin.unsqueeze(0).unsqueeze(2)
    else:  # inference mode: cos shape = (D/2,)
        cos = cos[None, None, None, :]  # (1, 1, 1, D/2)
        sin = sin[None, None, None, :]

    x_even = x1 * cos - x2 * sin  # (B, T, H, D/2)
    x_odd = x1 * sin + x2 * cos   # (B, T, H, D/2)

    x_out = torch.stack([x_even, x_odd], dim=-1).flatten(-2)  # (B, T, H, D)
